...
However, don't be to hard on yourself if you strugle with the content of the book: it might not be the method of learning from books, nor the quality of the books that's causing you trouble, but simply the fact that university math is quite different and quite harder then highschool math. 
...
We start by noting that $$2b^2 = a^2$$ implies that $$a^2$$ is evem which can only happen when $$a$$ is even.
Indeed $$(2k+1)^2 = 2(2k^2+2k) +1$$, so that sqaures of odd numbers are odd, and $$(2k)^2 = 2(2k^2)$$, so that squares of even numbers are even, hence $$a=2k$$ must be even.
But then $$2b^2 = a^2$$ becomes $$b^2 = 2k^2$$ so that by the same argument, $$b$$ is even as well.
This causes a contradiction when the numberator $$a$$ and the denominator $$b$$ of the fraction we chose to represent $$\sqrt 2$$ are coprime, so that they have no other common factors then $$1$$, as even numbers have common factor $$2$$.
Now, all fractions can be represented by a coprime nuberator and the denominator (cancel all common factors in a given representaton), so to avoid contradiction, the assumption that $$\sqrt 2$$ hẃas rational must have been false.
...
The largest distance two points can be at in such a square is the length of the sub-squares diagonal, which you can compute to be $$\sqrt 2 /2$$.

The step of subdividing the square into 4 required some initiative from your part.
...
However, if you dislike these moments too much then math may not be for you, and there is no sham in that.
Working on problems all day does not create a sane environnement.
Many mathematicians, even big names such as Riemann, Hadammard or Fulkerson, have suffered from depressions that may have been caused by causes other then math, but that math certainly didn't helpget over with.
I'm mentioning this because I have met people that I jugde to have been better off in terms of quality of life by doing something other then math.
...
I encourage you to think of it as a programming language: you have to talk to the computer that way because its just a buch of circuits.
...
Now that you've seen the difference that universtity math has to highschool math, you should be thrilled to start your studies, as this uni type math is much more exiting than the hoghschool math, but you should also be worried about how well you'll perform at uni type math.
...
My experiences is that the differences educational systems can make are astounding.
...
It is a difficult question to answer whether this truely matters. I've studied at the uni of Strasbourg, which isn't that prestigious, and at the uni of Bonn, which is rather prestigious.
My prefered system would be the free uni of Berlin, who's math faculty is quite good, but not famous. Carefully explaining this choice would take to long. 

#### READ BOOKS !

The next choice you'll have is to chose a learning resource for each of your classes.
Indeed, there is more then just the lecturer's script you can learn from.
...
#### Advice for the filthy students interested in applying math to actual problem from the real world

The choices of teaching content aren't in our favour.
Professors are either not intereseted in applications of their work, sometimes even applications with math itself, or they don't want to waste time on teaching about applications, as mathematical content is more important in a lecture.
To remedy this, I can only recommend finding the applications yourself. You can search the internet for them, or fick through books on disiplines from the natural and social sciences and look at the math they use, until you find a field that uses the type of math you're intersted in.
